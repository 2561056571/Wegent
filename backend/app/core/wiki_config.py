...
class WikiSettings(BaseSettings):
    ...
    CONTENT_WRITE_BASE_URL: str = "http://localhost:8000"  # Base server address for internal wiki content writer
    CONTENT_WRITE_ENDPOINT: str = "/api/internal/wiki/generations/contents"  # Fixed relative path for content writes
    DEFAULT_SECTION_TYPES: list[str] = ["overview", "architecture", "module", "api", "guide", "deep"]
    INTERNAL_API_TOKEN: str = "weki"  # Internal authentication token for content write API
