...
class WikiGeneration(WikiBase):
    ...
    id = Column(Integer, primary_key=True, index=True)
    project_id = Column(Integer, ForeignKey('wiki_projects.id', ondelete='CASCADE'),
                       nullable=False, index=True)
    user_id = Column(Integer, nullable=False, index=True)
    task_id = Column(Integer, nullable=True, index=True)
    team_id = Column(Integer, nullable=False)
    generation_type = Column(SQLEnum(WikiGenerationType), nullable=False, default=WikiGenerationType.FULL)
    source_snapshot = Column(JSON, nullable=False, comment='Source snapshot information')
    status = Column(SQLEnum(WikiGenerationStatus), nullable=False,
                   default=WikiGenerationStatus.PENDING, index=True)
...
class WikiContent(WikiBase):
    ...
    id = Column(Integer, primary_key=True, index=True)
    generation_id = Column(Integer, ForeignKey('wiki_generations.id', ondelete='CASCADE'),
                          nullable=False, index=True)
